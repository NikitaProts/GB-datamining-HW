...
class DateBase:
    ...
    def _get_or_create_comments(self, session, data: list) -> list:
        ...
        if data:
            for comment in data:
                ...
                result.extend(
                    self._get_or_create_comments(
                        session, comment["comment"]["children"])
                )
    ...
    def create_post(self, data):
        ...
        post = self._get_or_create(
            session, models.Post, "url", data["post_data"])
        author = self._get_or_create(
            session, models.Author, "url",  data["author_data"])
        tags = [self._get_or_create(session, models.Tag, "url", tag_data)
